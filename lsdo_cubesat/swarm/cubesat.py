...
class Cubesat(OptionsDictionary):
    ...
    def initialize(self):
        ...
        self.declare('approx_altitude_km', default=None, types=float, allow_none=True)
    ...
    def pre_setup(self):
        if self['approx_altitude_km'] is not None:
            self['acceleration_due_to_gravity'] = 9.81 * self['radius_earth_km'] ** 2 / (
                self['radius_earth_km'] + self['approx_altitude_km']) ** 2